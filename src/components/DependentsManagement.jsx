...
import { dependents, RELATIONSHIPS, validateDependent } from '../lib/dependents';
import { account } from '../lib/appwrite';
import { User, Calendar, Heart, Plus, Trash2, Edit2, X, Check, AlertCircle, Save, Loader } from 'lucide-react';
...
const DependentsManagement = () => {
    ...
    const loadData = async () => {
        try {
            ...
            setUser(currentUser);
            const data = await dependents.listByParent(currentUser.$id);
    ...
    const handleEdit = (dependent) => {
        setFormData({
            name: dependent.name,
            birth_date: dependent.birth_date.split('T')[0],
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                const updated = await dependents.update(editingId, formData);
                setMyDependents(prev => prev.map(d => d.$id === editingId ? updated : d));
            } else {
                const created = await dependents.create(user.$id, formData);
    ...
    return (
        <div className="space-y-6" dir="rtl">
            ...
            <div className="bg-gray-100 p-2 text-xs font-mono mb-4 border border-gray-300 rounded" dir="ltr">
                <p>Loading: {loading ? 'YES' : 'NO'}</p>
                <p>ShowForm: {showForm ? 'YES' : 'NO'}</p>
                <p>Error: {error || 'None'}</p>
                <p>Dependents: {myDependents.length}</p>
                <p>User ID: {user?.$id || 'None'}</p>
            </div>
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                    <Heart className="w-6 h-6 text-red-500" />
                    رعاياي (الأبناء والأقارب)
                </h2>
                {!showForm && (
                    <button
                    ...
                    <div key={dependent.$id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow relative group">
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mb-2">{dependent.relationship}</p>

                                <div className="flex flex-wrap gap-2 text-xs">
                                    <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-full flex items-center gap-1">
                                        <Calendar className="w-3 h-3" />
                                        {new Date(dependent.birth_date).toLocaleDateString('ar-MA')}
                                    </span>
